...
| 模块 | 当前能力 |
| --- | --- |
| 参与者入口 | 昵称登记与唯一性校验、浏览器绑定和昵称恢复、现场编号与个人 QR、查看本队资源和个人赛程、提交反馈。 |
| 现场编组 | Staff PIN 会话、按昵称或 `P-xxx` 搜索、扫码定位参与者、1–3 人队伍管理，以及不直接修改状态的自动分配预览和显式发布。 |
| 资源与资格 | Admin 导入 Workshop Code 和 Game Portal Code；Staff 按队发放、记录 Workshop 备注并确认参赛资格；Admin 可诊断资源、补发或回收 Code，并处理资格例外。 |
| 比赛运营 | Admin 冻结参赛名单、生成和调整小组赛、生成淘汰赛；Staff 录入赛果；系统计算积分并提供公开只读现场大屏。 |
| 运行基础 | 操作审计、阶段开关、SQLite 持久化、健康检查，以及分离的 feedback/live 部署配置。 |
