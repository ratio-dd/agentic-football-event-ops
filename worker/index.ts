...
interface Env { ASSETS: Fetcher; DB: D1Database; STAFF_PINS?: string; ADMIN_PIN?: string; EVENT_CONFIG?: string; }
...
const worker = {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (!url.pathname.startsWith("/api/")) {
      const assetPath = url.pathname === "/display" ? "/display.html" : ["/", "/staff", "/ta", "/admin"].includes(url.pathname) ? "/index.html" : "";
    ...
    try {
      ...
      if (pathname === "/api/participant/help-requests" && request.method === "POST") return mutation(env, request, (s) => createHelpRequest(s, request));
      ...
      if (/^\/api\/ops\/help-requests\/[^/]+\/claim$/.test(pathname) && request.method === "POST") return mutation(env, request, (s) => claimHelpRequest(s, helpRequestId(pathname), staff));
      if (/^\/api\/ops\/help-requests\/[^/]+\/resolve$/.test(pathname) && request.method === "POST") return mutation(env, request, (s) => resolveHelpRequest(s, helpRequestId(pathname), staff));
...
function eventDefinition(env: Env) {
  if (!env.EVENT_CONFIG) throw new Error("请求缺少 tenant scope");
  return normaliseEventDefinition(JSON.parse(env.EVENT_CONFIG));
}
function eventState(definition: any) {
  return { id: definition.id, tenantId: definition.id, name: definition.name, branding: definition.branding, teamPolicy: definition.teamPolicy, tournamentPolicy: definition.tournamentPolicy, maxWorkshopTeams: definition.teamPolicy.maxTeams, workshopUrl: definition.links.workshopUrl, gamePortalUrl: definition.links.gamePortalUrl, gates: { ...definition.defaultGates }, publicMaintenanceSnapshotEnabledAt: null };
}
async function stateOf(env: Env, definition = eventDefinition(env)) {
  const row = await env.DB.prepare("SELECT data, version FROM event_state WHERE id = ?").bind(definition.id).first<{ data: string; version: number }>();
  if (row) return { state: normalise(JSON.parse(row.data), definition), version: row.version };
  const state = initialState(definition);
  await env.DB.prepare("INSERT OR IGNORE INTO event_state (id, data, version, updated_at) VALUES (?, ?, 1, ?)").bind(definition.id, JSON.stringify(state), now()).run();
  return stateOf(env, definition);
}
async function mutation(env: Env, request: Request, action: (s: State) => any): Promise<Response> {
  const definition = eventDefinition(env);
  for (let retry = 0; retry < 5; retry += 1) {
    const { state, version } = await stateOf(env, definition); const result = await action(state);
    ...
    const write = await env.DB.prepare("UPDATE event_state SET data = ?, version = ?, updated_at = ? WHERE id = ? AND version = ?")
      .bind(JSON.stringify(state), version + 1, now(), definition.id, version).run();
    if (write.meta.changes) return json({ ...result, version: version + 1 });
  }
  return json({ error: "现场数据刚刚发生变化，请重试" }, 409);
}

function initialState(definition: any) { return { event: eventState(definition), participants: [], teams: [], allocationRuns: [], manualSplitAudits: [], workshopCodes: [], gamePortalCodes: [], competition: { frozenTeamIds: [], frozenAt: null, frozenBy: null }, staffAccounts: [], staffSessions: [], adminSessions: [], tournament: null, auditLog: [], feedback: [], helpRequests: [] }; }
function normalise(raw: any, definition: any): State {
  ...
  const helpRequests = Array.isArray(raw?.helpRequests) ? raw.helpRequests.map((request: any) => ({ id: request.id || id(), participantId: text(request.participantId, 80), teamId: text(request.teamId, 80) || null, category: ["workshop_access", "game_portal", "team", "other"].includes(request.category) ? request.category : "other", status: ["open", "claimed", "resolved"].includes(request.status) ? request.status : "open", createdAt: request.createdAt || now(), claimedAt: request.claimedAt || null, claimedById: request.claimedById || null, claimedByNickname: text(request.claimedByNickname, 24) || null, resolvedAt: request.resolvedAt || null, resolvedById: request.resolvedById || null, resolvedByNickname: text(request.resolvedByNickname, 24) || null })).filter((request: any) => request.participantId) : [];
  return { ...base, ...raw, event: { ...base.event, workshopUrl: raw?.event?.workshopUrl || base.event.workshopUrl, gamePortalUrl: raw?.event?.gamePortalUrl || base.event.gamePortalUrl, gates: { ...base.event.gates, ...(raw?.event?.gates || {}) }, publicMaintenanceSnapshotEnabledAt: raw?.event?.publicMaintenanceSnapshotEnabledAt || null }, participants, teams, allocationRuns, manualSplitAudits: Array.isArray(raw?.manualSplitAudits) ? raw.manualSplitAudits : [], workshopCodes, gamePortalCodes, competition: { ...base.competition, ...(raw?.competition || {}) }, staffAccounts: Array.isArray(raw?.staffAccounts) ? raw.staffAccounts : base.staffAccounts, staffSessions: Array.isArray(raw?.staffSessions) ? raw.staffSessions : [], adminSessions: Array.isArray(raw?.adminSessions) ? raw.adminSessions : [], auditLog: Array.isArray(raw?.auditLog) ? raw.auditLog : [], feedback: Array.isArray(raw?.feedback) ? raw.feedback : [], helpRequests };
}
function teamPolicy(s: State) { return s.event.teamPolicy; }
function tournamentPolicy(s: State) { return s.event.tournamentPolicy; }
...
async function participantQr(env: Env, request: Request, url: URL) { const { state } = await stateOf(env); const shortId = text(url.searchParams.get("participant"), 20).toUpperCase(); const participant = state.participants.find((p: any) => p.staffShortId === shortId) || state.participants.find((p: any) => p.clientIds.includes(client(request))); if (!participant) return new Response("Not found", { status: 404 }); const svg = await QRCode.toString(participant.staffShortId, { type: "svg", errorCorrectionLevel: "M", margin: 1, color: { dark: "#075b32", light: "#ffffff" } }); return new Response(svg, { headers: { "content-type": "image/svg+xml", "cache-control": "no-store" } }); }
...
function helpRequestId(path: string) { return after(path, "help-requests"); }
...
function helpRequestView(s: State, request: any, staffAccess = false) {
  const participant = s.participants.find((candidate: any) => candidate.id === request.participantId); const requestTeam = request.teamId ? team(s, request.teamId) : null;
  const view: any = { id: request.id, category: request.category, status: request.status, createdAt: request.createdAt, claimedAt: request.claimedAt, claimedByNickname: request.claimedByNickname, resolvedAt: request.resolvedAt };
  if (staffAccess) Object.assign(view, { participantId: request.participantId, participantNickname: participant?.nickname || "未知参与者", participantShortId: participant?.staffShortId || "", teamId: request.teamId, teamNumber: requestTeam?.teamNumber || null, resolvedByNickname: request.resolvedByNickname });
  return view;
}
function sortedHelpRequests(s: State) {
  const rank: Record<string, number> = { open: 0, claimed: 1, resolved: 2 };
  return [...s.helpRequests].sort((first: any, second: any) => rank[first.status] - rank[second.status] || second.createdAt.localeCompare(first.createdAt));
}
...
function claimHelpRequest(s: State, value: string, actor: Staff) {
  const request = s.helpRequests.find((item: any) => item.id === value); if (!request) return fail("求助请求不存在", 404);
  if (request.status !== "open") return fail("只有待接单的求助可以接单", 409);
  request.status = "claimed"; request.claimedAt = now(); request.claimedById = actor.id; request.claimedByNickname = actor.nickname;
  audit(s, actor, "help_request.claimed", "helpRequest", request.id, request.category); return { helpRequest: helpRequestView(s, request, true) };
}
function resolveHelpRequest(s: State, value: string, actor: Staff) {
  const request = s.helpRequests.find((item: any) => item.id === value); if (!request) return fail("求助请求不存在", 404);
  if (request.status !== "claimed") return fail("求助必须先接单再标记解决", 409);
  request.status = "resolved"; request.resolvedAt = now(); request.resolvedById = actor.id; request.resolvedByNickname = actor.nickname;
  audit(s, actor, "help_request.resolved", "helpRequest", request.id, request.category); return { helpRequest: helpRequestView(s, request, true) };
}
...
async function staffFor(env: Env, request: Request): Promise<Staff | null> {
  ...
  const { state } = await stateOf(env); const session = state.staffSessions.find((x: any) => x.token === token); return session ? { id: session.staffAccountId, nickname: session.nickname } : null;
}
async function adminFor(env: Env, request: Request): Promise<Staff | null> {
  ...
  const { state } = await stateOf(env); const session = state.adminSessions.find((x: any) => x.token === token && x.staffSessionToken === staffToken);
...
async function publicView(env: Env, request: Request) { const { state } = await stateOf(env); const clientId = client(request); const participant = state.participants.find((p: any) => p.clientIds.includes(clientId)); const currentTeam = participant?.teamId ? teamView(state, team(state, participant.teamId), clientId) : null; const helpRequests = participant ? sortedHelpRequests(state).filter((item: any) => item.participantId === participant.id).slice(0, 10).map((item: any) => helpRequestView(state, item)) : []; return { event: state.event, currentParticipant: participant ? publicParticipant(participant, clientId) : null, currentTeam, tournament: publicTournament(state), helpRequests }; }
async function staffView(env: Env) { const { state } = await stateOf(env); const active = state.teams.filter((t: any) => t.status !== "dissolved"); const manualTeams = active.filter((t: any) => t.type === "manual"); const automaticTeams = active.filter((t: any) => t.type === "auto"); const freePeople = state.participants.filter((p: any) => !p.teamId && p.allocationSource !== "waitlist"); const waitlist = state.participants.filter((p: any) => p.allocationSource === "waitlist"); const conflicts = allocationValidation(state).conflicts; return { event: state.event, codeSummary: codeSummary(state), competition: state.competition, participants: state.participants.map((p: any) => ({ ...p, clientIds: undefined, codeVisibleClientIds: undefined })), teams: state.teams.map((t: any) => teamView(state, t, "", true)), helpRequests: sortedHelpRequests(state).slice(0, 200).map((item: any) => helpRequestView(state, item, true)), allocation: { resourceLimit: teamPolicy(state).maxTeams, freePeople: freePeople.map((p: any) => publicParticipant(p, "")), waitlist: waitlist.map((p: any) => ({ ...publicParticipant(p, ""), waitlistReason: p.waitlistReason || "CAPACITY_EXHAUSTED" })), manualTeams: manualTeams.map((t: any) => teamView(state, t, "", true)), automaticTeams: automaticTeams.map((t: any) => teamView(state, t, "", true)), conflicts, lowAttendanceSuggestions: lowAttendanceSuggestions(state), latestPreview: state.allocationRuns.filter((r: any) => r.status === "preview").at(-1) || null, latestPublished: state.allocationRuns.filter((r: any) => r.status === "published").at(-1) || null, runs: state.allocationRuns.slice(-20).reverse(), manualSplitAudits: state.manualSplitAudits.slice(-50).reverse() }, tournament: publicTournament(state), auditLog: state.auditLog.slice(-100).reverse() }; }
async function adminView(env: Env) { const staff = await staffView(env); const { state } = await stateOf(env); const reclaimableTeams = state.teams.filter((team: any) => team.status === "dissolved" && teamHasIssuedCodes(team)).map((team: any) => teamView(state, team, "", true)); const gamePortalBackfillTeams = gamePortalBackfillTargets(state).map((team: any) => teamView(state, team, "", true)); return { ...staff, feedback: state.feedback.slice(-100).reverse(), reclaimableTeams, gamePortalBackfillTeams }; }
async function adminDiagnostics(env: Env) {
  const { state, version } = await stateOf(env); const active = state.teams.filter((item: any) => item.status !== "dissolved");
...
async function publicMaintenanceSnapshot(env: Env) {
  const { state, version } = await stateOf(env);
...
async function makeTeam(s: State, request: Request, actor: Staff) {
  const b = await body(request); const memberIds = ids(b.memberIds); const policy = teamPolicy(s); if (memberIds.length < policy.minMembers || memberIds.length > policy.maxMembers) return fail(`每队必须为 ${policy.minMembers}–${policy.maxMembers} 人`);
  ...
  const finalMemberIds = [...new Set([...t.memberIds, ...memberIds])]; if (finalMemberIds.length > policy.maxMembers) return fail(`每队必须为 ${policy.minMembers}–${policy.maxMembers} 人`);
...
function dispatchAssignment(s: State, b: any, actor: Staff) {
  const participantIds = ids(b.participantIds); const targetValue = text(b.targetTeamId, 80); const createTarget = targetValue === "new"; const policy = teamPolicy(s);
  if (participantIds.length < 1 || participantIds.length > policy.maxMembers) return fail(`一次只能调度 1–${policy.maxMembers} 人`);
  ...
  if (finalTargetIds.length > policy.maxMembers) return fail(`目标队伍容量不足：当前操作后将有 ${finalTargetIds.length} 人，最多 ${policy.maxMembers} 人`, 409);
...
async function updateTeam(s: State, value: string, request: Request, actor: Staff) {
  const t = team(s, value); const b = await body(request); const memberIds = ids(b.memberIds); const policy = teamPolicy(s);
  if (!t || t.status === "dissolved") return fail("队伍不存在或已解散", 404);
  if (memberIds.length < policy.minMembers || memberIds.length > policy.maxMembers) return fail(`每队必须为 ${policy.minMembers}–${policy.maxMembers} 人`);
...
function allocationValidation(s: State) {
  const conflicts: any[] = []; const owners = new Map<string, string>(); const manual = activeManualTeams(s); const policy = teamPolicy(s);
  if (manual.length > policy.maxTeams) conflicts.push({ code: "MANUAL_TEAM_LIMIT", message: `人工队数量为 ${manual.length}，超过资源上限 ${policy.maxTeams}` });
  manual.forEach((t: any) => {
    const core = ids(t.coreMemberIds?.length ? t.coreMemberIds : t.memberIds);
    if (core.length < policy.minMembers || core.length > policy.maxMembers) conflicts.push({ code: "MANUAL_TEAM_SIZE", teamId: t.id, message: `${t.teamNumber} 的人工核心必须为 ${policy.minMembers}–${policy.maxMembers} 人` });
...
function lowAttendanceSuggestions(s: State) {
  if (s.participants.length >= teamPolicy(s).maxTeams) return [];
...
function buildAllocation(s: State, seed: string) {
  ...
  while (free.length && manualTeams.length + autoTeams.length < policy.maxTeams) autoTeams.push({ id: `preview-auto-${serial++}`, type: "auto", teamNumber: `AUTO-${String(autoTeams.length + 1).padStart(2, "0")}`, coreMemberIds: [], allocatedMemberIds: [], memberIds: [free.shift()], resourceSlot: null, tieBreak: random() });
  const fill = (targets: any[], includeManual = false) => {
    while (free.length) {
      const candidates = targets.filter((t: any) => t.memberIds.length < policy.maxMembers);
  ...
  const waitlist = free.map((participantId) => ({ participantId, reason: manualTeams.length + autoTeams.length >= policy.maxTeams ? "CAPACITY_EXHAUSTED" : "MANUAL_TEAMS_CLOSED" }));
...
async function splitManualTeam(s: State, value: string, request: Request, actor: Staff) {
  ...
  const policy = teamPolicy(s); const original = ids(source.coreMemberIds); if (!groups.length || groups.some((group: string[]) => group.length > policy.maxMembers) || ids(groups.flat()).length !== original.length || original.some((personId: string) => !ids(groups.flat()).includes(personId))) return fail(`拆分结果必须完整覆盖原人工核心成员，每队 ${policy.minMembers}–${policy.maxMembers} 人`, 409);
...
async function updateTeamStatus(s: State, value: string, request: Request, actor: Staff) {
  ...
  const next = text((await body(request)).status, 40); const allowed = ["draft", "ready_code", "issued"];
  if (next === "ta_qualified") return fail("可参赛状态只能通过专用的 TA 确认动作设置", 409);
  if (!allowed.includes(next)) return fail("状态无效", 400);
  if (t.qualificationStatus === "ta_qualified") return fail("已确认的参赛资格只能由 Admin 撤销", 409);
  if (next === "issued" && !t.workshopCodeId) return fail("未发放 Workshop Code 的队伍不能设为 Workshop 中", 409);
  ...
  t.status = next;
  t.qualificationStatus = "not_qualified"; t.qualifiedAt = null; if (next === "issued") t.workshopStatus = "in_progress"; else t.workshopStatus = "not_started";
...
async function updateEventGates(s: State, request: Request, actor: Staff) {
  ...
  const next = { ...s.event.gates }; Object.keys(next).forEach((key) => { if (typeof gates[key] === "boolean") next[key] = gates[key]; });
...
async function generateTournament(s: State, request: Request, actor: Staff) {
  ...
  const policy = tournamentPolicy(s); const requestedGroupCount = Number(b.groupCount); const defaultGroupCount = Math.min(policy.maxGroups, Math.ceil(teams.length / policy.maxTeamsPerGroup)); const groupCount = Math.max(1, Math.min(policy.maxGroups, Number.isInteger(requestedGroupCount) && requestedGroupCount > 0 ? requestedGroupCount : defaultGroupCount)); const qualifiersPerGroup = Math.max(1, Math.min(policy.maxQualifiersPerGroup, Number(b.qualifiersPerGroup) || policy.defaultQualifiersPerGroup));
  const minimumGroupCount = Math.ceil(teams.length / policy.maxTeamsPerGroup);
  if (groupCount < minimumGroupCount) return fail(`${teams.length} 支队伍至少需要 ${minimumGroupCount} 个小组；每组最多 ${policy.maxTeamsPerGroup} 队`, 409);
...
function validateTournamentGroups(s: State, tournament: any, rawGroups: unknown) {
  ...
  const maximum = tournamentPolicy(s).maxTeamsPerGroup;
  if (groups.some((group: any) => !group.id || group.teamIds.length < 1 || group.teamIds.length > maximum)) return fail(`每个小组必须有 1–${maximum} 支队伍`, 409);
...
async function updateTournamentGroups(s: State, request: Request, actor: Staff) {
  ...
  const validated = validateTournamentGroups(s, tournament, (await body(request)).groups); if (validated.error) return validated;
