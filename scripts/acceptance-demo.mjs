...
async function importCodes(sessions, count = 32) {
  const gamePortalCodes = Array.from({ length: count }, (_, index) => `ACCEPT-PORTAL-${scenario.toUpperCase()}-${String(index + 1).padStart(2, "0")}`);
  await api("/api/ops/codes/import", { method: "POST", staff: sessions.staff, admin: sessions.admin, body: { gamePortalCodes } });
