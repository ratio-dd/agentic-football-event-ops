...
function render(data) {
  ...
  root.innerHTML = `<header class="display-header"><div><p>${e(data.event?.name || "AGENTIC FOOTBALL")}</p><h1>${tournament.status === "knockout" ? "淘汰赛" : `小组赛 · 第 ${tournament.currentGroupRound || 1} / ${tournament.totalGroupRounds || 1} 轮`}</h1></div><span>现场大屏 · 自动更新</span></header><section class="next-match"><p>下一场</p><strong>${ready ? `${ready.stage === "group" ? `${e(ready.groupId)} 组 · 第 ${ready.round || 1} 轮 · ` : `第 ${ready.round} 轮 · `}${e(ready.teamALabel)} <i>vs</i> ${e(ready.teamBLabel)}` : "等待 Admin 开放下一轮"}</strong></section><section class="display-grid">${tournament.groups.map(standings).join("")}</section><section class="display-results"><h2>本轮赛果</h2><ul>${groupResults.map(matchLine).join("") || "<li><span>比赛即将开始</span></li>"}</ul></section>${tournament.knockoutMatches?.length ? `<section class="display-bracket"><h2>淘汰赛对阵</h2><div>${tournament.knockoutMatches.map(bracket).join("")}</div></section>` : ""}`;
}
