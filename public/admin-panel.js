...
export function renderAdmin(root, state, api, controls) {
  ...
  root.innerHTML = `<div class="admin-shell"><header class="admin-top"><div><strong>⚽ 管理后台</strong><small>高影响操作与现场例外</small></div><div class="top-actions"><button class="text-button" data-feedback>反馈</button><button class="text-button" data-action="return-staff">返回 Staff</button></div></header><main><section class="admin-heading"><p class="eyebrow">Admin</p><h1>管理例外，不干扰现场</h1><p>日常人员、队伍、发码、练习赛确认与赛果录入仍在 Staff 工作台完成。</p></section><nav class="admin-nav">${[["activity", "活动设置"], ["resources", "资源管理"], ["competition", "比赛管理"], ["records", "反馈与记录"]].map(([id, label]) => `<button data-admin-section="${id}" class="${section === id ? "active" : ""}">${label}</button>`).join("")}</nav><section class="admin-panel">${sections[section]}</section></main><p class="notice" aria-live="polite"></p></div>`;
