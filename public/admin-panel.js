...
export function renderAdmin(root, state, api, controls) {
  ...
  const sections = { activity: activityPanel(state), resources: resourcesPanel(state), competition: competitionPanel(state, ui), records: recordsPanel(state) };
  ...
  root.querySelector("#admin-generate-groups")?.addEventListener("submit", (event) => { event.preventDefault(); const form = new FormData(event.target); return action(async () => { await api.tournament(Number(form.get("groupCount")), Number(form.get("qualifiersPerGroup"))); ui.competitionNotice = "小组赛已生成。Staff 可从“比赛”录入赛果；现场大屏已可打开。"; }, "小组赛赛程已生成。", event.target.querySelector("button")); });
...
function activityPanel(state) {
  ...
  return `<section class="admin-section"><div><h2>链接与阶段开关</h2><p>链接会同步到已领 Code 的参赛者页面；关闭入口后，Staff 无法执行对应阶段动作。</p></div><form id="admin-event-settings" class="admin-form"><label>Workshop 链接<input name="workshopUrl" type="url" required value="${e(state.event.workshopUrl)}"></label><label>Game Portal 链接<input name="gamePortalUrl" type="url" required value="${e(state.event.gamePortalUrl)}"></label><fieldset><legend>活动阶段</legend>${row("selfServiceTeam", "临时开放自助组队", "默认关闭；仅配合专用入口使用")}${row("codeIssuance", "允许发放 Code", "关闭后，Staff 不能向新队伍发放资源")}${row("qualification", "允许确认参赛资格", "关闭后，TA 不能确认新的比赛队伍")}${row("scheduleEditing", "允许赛程编辑", "关闭后，不能冻结、调整或作废赛程")}</fieldset><button>保存活动设置</button></form></section>`;
}
...
function competitionPanel(state, ui) {
  ...
  return `<section class="admin-section">${progress}<div><h2>${tournament.status === "knockout" ? "淘汰赛已生成" : "小组赛已生成"}</h2><p>Staff 录入赛果；这里处理分组调整、淘汰赛生成和作废。</p></div><div class="admin-competition-links"><a href="/staff">打开 Staff 赛果录入</a><a href="/display" target="_blank" rel="noreferrer">打开现场大屏</a></div>${tournament.status === "group" && !tournament.matches.some((match) => match.status === "completed") ? `<form id="admin-swap-groups" class="admin-form"><h3>调整未开赛分组</h3><label>第一支队伍<select name="firstTeamId">${teamOptions(teams, first)}</select></label><label>第二支队伍<select name="secondTeamId">${teamOptions(teams, second)}</select></label><button>交换两队位置</button></form>` : ""}${tournament.status === "group" && !tournament.knockoutMatches?.length ? `<form id="admin-generate-knockout" class="admin-form"><button ${tournament.matches.some((match) => match.status !== "completed") ? "disabled" : ""}>生成淘汰赛</button></form>` : ""}<button data-action="void-tournament" class="secondary">作废并重新分组</button>${qualificationExceptions(eligible)}</section>`;
}
