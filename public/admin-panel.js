...
export function renderAdmin(root, state, api, controls) {
  ...
  root.querySelector("#admin-import-codes")?.addEventListener("submit", (event) => { event.preventDefault(); const form = new FormData(event.target); const parse = (name) => String(form.get(name) || "").split(/\r?\n/).map((code) => code.trim()).filter(Boolean); const workshopCodes = parse("workshopCodes"), gamePortalCodes = parse("gamePortalCodes"); return action(() => api.importResourceCodes({ workshopCodes, gamePortalCodes }), (result) => { const workshop = result.imported.workshop, gamePortal = result.imported.gamePortal; const skipped = workshop.duplicates + gamePortal.duplicates; return `已新增 Workshop ${workshop.added} 个、Game Portal ${gamePortal.added} 个${skipped ? `；跳过重复 ${skipped} 个` : ""}。`; }, event.target.querySelector("button")); });
  root.querySelector("#admin-batch-issue")?.addEventListener("submit", (event) => { event.preventDefault(); const teamIds = [...new FormData(event.target).getAll("teamId")]; if (!teamIds.length) { notice.textContent = "请至少选择一支待发放队伍。"; return; } return action(() => api.batchIssueCodes(teamIds), `已向 ${teamIds.length} 支队伍同时发放两组 Code。`, event.target.querySelector("button[type='submit']")); });
...
function resourcesPanel(state, ui) {
  ...
  const unissued = (state.teams || []).filter((team) => team.status !== "dissolved" && !team.workshopCodeId && !team.gamePortalCodeId);
  ...
  const batch = `<section class="admin-subsection"><h3>批量发放两组 Code</h3><p>只列出有效且尚未发放的队伍。发放为原子操作：任一库存不足时整批不执行。</p><form id="admin-batch-issue" class="admin-form"><div><button type="button" class="secondary" data-action="batch-select-all">取消全选</button></div>${unissued.map((team) => `<label class="check-row"><input type="checkbox" name="teamId" value="${e(team.id)}" checked>${members(team)}</label>`).join("") || '<p class="hint">暂无待发放队伍。</p>'}<button type="submit" ${!unissued.length ? "disabled" : ""}>向所选队伍发放 Workshop + Game Portal Code</button></form></section>`;
  return `<section class="admin-section"><div><h2>官方 Code 与回收</h2><p>导入只创建可用资源；发放后才会与队伍绑定。</p><button type="button" class="secondary" data-action="resource-diagnostics">只读核对当前资源</button></div>${diagnosticCard}<section class="admin-metrics">${metric("Workshop 可用", summary.workshop)}${metric("Game Portal 可用", summary.gamePortal)}<article><span>可同时发放</span><strong>${summary.pairsAvailable}</strong><small>两组 Code 均有库存</small></article></section><form id="admin-import-codes" class="admin-form"><label>Workshop Code<textarea name="workshopCodes" rows="5" placeholder="每行一个真实 Workshop Code"></textarea></label><label>Game Portal Code<textarea name="gamePortalCodes" rows="5" placeholder="每行一个真实 Game Portal Code"></textarea></label><button>导入已填写的 Code</button></form>${batch}<section class="admin-subsection"><h3>补发旧队的 Game Portal Code</h3><p>${backfill.length ? `有 ${backfill.length} 支有效队伍已领 Workshop Code、尚未领 Game Portal Code。将按原 Workshop Code 发放顺序补发。` : "没有需要补发的旧队。"}</p>${backfill.length ? `<p class="hint">本次可补发 ${Math.min(backfill.length, summary.gamePortal.available)} 支；剩余 ${Math.max(0, backfill.length - summary.gamePortal.available)} 支会保留在待补发列表。</p><button data-action="backfill-game-portal-codes" ${canBackfill ? "" : "disabled"}>批量补发 Game Portal Code</button>` : ""}</section><section class="admin-subsection"><h3>待处理的已消耗 Code</h3><div class="admin-list">${reclaimable.map((team) => `<article><div><strong>${members(team)}</strong><small>队伍已解散；已发放 Code 仍保留为已消耗。</small></div><button data-action="reclaim-code" data-team-id="${e(team.id)}" class="secondary">回收 Code</button></article>`).join("") || '<p class="hint">暂无待回收 Code。</p>'}</div></section></section>`;
}
...
function resetPanel(state) {
  ...
  const archiveSummary = latest ? `<div class="admin-reset-history"><strong>最近归档</strong><span>${e(latest.eventName)} · ${e(new Date(latest.archivedAt).toLocaleString("zh-CN"))}</span><small>${latest.counts.participants} 位参与者 · ${latest.counts.teams} 支队伍 · Workshop ${latest.counts.workshopCodes} 个 · Game Portal ${latest.counts.gamePortalCodes} 个</small></div>` : '<p class="hint">当前还没有归档记录。</p>';
