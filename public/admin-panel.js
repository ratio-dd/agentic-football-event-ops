...
function activityPanel(state) {
  ...
  return `<section class="admin-section"><div><h2>链接与阶段开关</h2><p>链接会同步到已领 Code 的参赛者页面；关闭入口后，Staff 无法执行对应阶段动作。</p></div><form id="admin-event-settings" class="admin-form"><label>Workshop 链接<input name="workshopUrl" type="url" required value="${e(state.event.workshopUrl)}"></label><label>Game Portal 链接<input name="gamePortalUrl" type="url" required value="${e(state.event.gamePortalUrl)}"></label><fieldset><legend>活动阶段</legend>${row("selfServiceTeam", "临时开放自助组队", "默认关闭；仅配合专用入口使用")}${row("codeIssuance", "允许发放 Code", "关闭后，Staff 不能向新队伍发放资源")}${row("qualification", "允许确认参赛资格", "关闭后，TA 不能确认新的比赛队伍")}${row("scheduleEditing", "允许赛程编辑", "关闭后，不能冻结、调整或作废赛程")}</fieldset><fieldset><legend>临时维护</legend>${row("publicMaintenanceSnapshot", "公开脱敏维护快照", "默认关闭；开启后只列出队伍状态、人数与资源计数，不包含 Code/PIN")}</fieldset><button>保存活动设置</button></form></section>`;
}
