...
export function renderParticipant(root, state, api, refresh) {
  ...
  root.querySelector("#copy-team-code")?.addEventListener("click", async () => { try { await navigator.clipboard.writeText(team.teamCode); notice.textContent = "Team Code 已复制。"; } catch { notice.textContent = "无法自动复制，请长按 Code 后复制。"; } });
